...
public class RobotStateGenerator extends Thread  {
    ...
    public void run() {
        while (!stop) {
            double time = GameScreen.getTime(); //TODO: Was Timer.getFPGA
    ...
    public float getLeftVelocityInchesPerSec() {
        return (float) (getLeftDistanceInches() - left_encoder_prev_distance_) * (1000f / (GameScreen.getTime() - lastLoop));
    }

    public float getRightVelocityInchesPerSec() {
        return (float) (getRightDistanceInches() - right_encoder_prev_distance_) * (1000f / (GameScreen.getTime() - lastLoop));
    }
}
