...
public class PIDController implements Runnable {
    ...
    public void log() {
        if (enabled)
            Utils.log("Target: " + getTarget() + "; Current Value: " + getCurrentSourceValue() + "; Motor Setpoint: " + getMotorPower());
    }
    ...
    public void logVerbose() {
        if (enabled)
            synchronized (verboseLogLock) {
                /*
                Log.t("P: " + Utils.roundToPlace(p*kP, 3)
                        + ", I: " + Utils.roundToPlace(i*kI, 3)
        ...
        while (true) {
            if (enabled/* && RobotCore.isEnabled()*/) {
                synchronized (verboseLogLock) {
                    ...
                    i = Math.abs(i * kI) > 1 ? (max / kI) * Utils.sign((float)i) : i; //Prevent i windup

                    double setpoint = p * kP + i * kI - d * kD;
                    setpoint = Utils.minMax((float)setpoint, 0, 1);

                    power = Utils.minMax((float)setpoint, (float)min, (float)max);
                    ...
                    if (Utils.sign((float)power) != Utils.sign((float)p)) {
                        //power = 0;
                        //Log.i("Sign isn't equal to P. NOT compensating by setting motor power to 0.");
                    }

                    if ((Utils.sign((float)i) != Utils.sign((float)p)) && isRotational) {
                        i = 0;
                    ...
                    if ((Utils.sign((float)p) != startingSign) && shouldInstaKill) disable();
